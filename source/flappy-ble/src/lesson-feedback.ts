...
export function explainDifference(expected: Action[], actual: Action[], path = ''): string {
  for (let i = 0; i < Math.max(expected.length, actual.length); i++) {
    const want = expected[i], got = actual[i], location = `${path}action ${i + 1}`;
    if (!want) return `${location}: remove the extra ${describe(got)} block.`;
    if (!got) return `${location}: add ${describe(want)}.`;
