...
export class Link {
  ...
  async init(): Promise<void> {
    const off = await this.sdk.device.watchState(s => {
      ...
      if (this.reader) {
        ...
        const a = s.activity;
        if (a && !(a.owner.type === 'module' && (a.owner.id === this.context.module.id || a.owner.id === this.context.module.instanceId) && a.kind === 'send')) this.reader.abort('Readback canceled: conflicting shared-device activity.');
      }
      this.changed();
    });
  ...
  async current(expected?: string): Promise<{ state: State; target: Device }> {
    ...
    if (target.profileId !== PROFILE) throw new Error('This game requires the Crowbot compatibility firmware/profile.');
