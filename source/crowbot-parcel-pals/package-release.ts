...
const manifest=JSON.parse(await readFile(join(root,'module.json'),'utf8'));if(manifest.id!=='crowbot-parcel-pals'||manifest.version!=='1.1.0')throw new Error('Wrong manifest');
await mkdir('../../downloads',{recursive:true});
execFileSync('python3',['-c',`import pathlib,zipfile,stat
root=pathlib.Path('../../crowbot-parcel-pals')
zip_path=pathlib.Path('../../downloads/crowbot-parcel-pals-1.1.0.zip')
...
`],{stdio:'inherit'});
const zip=await readFile('../../downloads/crowbot-parcel-pals-1.1.0.zip');await writeFile(join(results,'archive.json'),JSON.stringify({files,uncompressedBytes:bytes,zipBytes:zip.length,sha256:createHash('sha256').update(zip).digest('hex')},null,2));
