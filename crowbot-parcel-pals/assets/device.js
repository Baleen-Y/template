...
export class Link {
    ...
    async init() {
        const off = await this.sdk.device.watchState(s => {
            ...
            if (this.reader) {
                ...
                const a = s.activity;
                if (a && !(a.owner.type === 'module' && (a.owner.id === this.context.module.id || a.owner.id === this.context.module.instanceId) && a.kind === 'send'))
    ...
    async current(expected) {
        ...
        if (target.profileId !== PROFILE)
            throw new Error('This game requires the Crowbot compatibility firmware/profile.');
        if (state.activity)
