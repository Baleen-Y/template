...
export function runtimeCommand(tag, operation, nonce, index = 0) {
    if (!/^[a-f0-9]{8,16}$/.test(tag) || !/^[a-f0-9]{6}$/.test(nonce))
        throw new Error('Invalid runtime routing identifier.');
    if (!['a', 's', 't'].includes(operation) || !Number.isInteger(index) || index < 0 || index > 255)
