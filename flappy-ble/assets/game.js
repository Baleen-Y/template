...
export class Simulation {
    constructor(onPipe, onEnd, random = Math.random) {
        this.onPipe = onPipe;
        this.onEnd = onEnd;
        this.random = random;
        this.stage = STAGES[0];
    ...
    down() { if (this.running)
        this.velocity = Math.max(220, this.velocity + 230); }
    ...
    update(dt) {
        ...
        for (let i = 0; i < steps && this.running; i++)
            this.step(Math.min(dt, 0.06) / steps);
    }
    step(dt) {
        this.elapsed += dt;
...
export class GameView {
    constructor(canvas, onPipe, onEnd) {
        this.canvas = canvas;
        this.frameId = 0;
        this.last = 0;
        this.visible = true;
        ...
        this.frame = (time) => {
            if (this.disposed)
                return;
            if (this.visible) {
                this.sim.update(this.last ? (time - this.last) / 1000 : 0);
                this.draw();
            }
            this.last = time;
        ...
        if (!ctx)
            throw new Error('Canvas 2D context could not be created. Retry graphics or reopen the module.');
        this.ctx = ctx;
        this.sim = new Simulation(onPipe, onEnd);
        this.draw();
        this.frameId = requestAnimationFrame(this.frame);
    }
    ...
    draw() {
        ...
        sky.addColorStop(0, '#70d3f4');
        sky.addColorStop(1, '#e4f7ee');
        c.fillStyle = sky;
        c.fillRect(0, 0, 900, 520);
        c.fillStyle = 'rgba(255,255,255,0.75)';
        for (let i = 0; i < 5; i++) {
            const x = (i * 217 - s.elapsed * 9 + 1100) % 1100 - 100, y = 60 + (i % 3) * 65;
            c.beginPath();
            c.ellipse(x, y, 53, 17, 0, 0, Math.PI * 2);
            c.ellipse(x + 12, y - 13, 28, 23, 0, 0, Math.PI * 2);
            c.fill();
        }
        const colors = ['#269f84', '#477ac6', '#8160bd'];
        for (const p of s.pipes) {
            const top = p.center - s.stage.gap / 2, bottom = p.center + s.stage.gap / 2;
            c.fillStyle = colors[s.stage.id - 1];
            c.fillRect(p.x, 0, 70, top);
            c.fillRect(p.x, bottom, 70, 494 - bottom);
            c.fillRect(p.x - 5, top - 20, 80, 20);
            c.fillRect(p.x - 5, bottom, 80, 20);
            c.fillStyle = 'rgba(255,255,255,.25)';
            c.fillRect(p.x + 9, 0, 8, top - 20);
            c.fillRect(p.x + 9, bottom + 20, 8, 494 - bottom);
        }
        c.fillStyle = '#63b26b';
        c.fillRect(0, 494, 900, 26);
        c.fillStyle = '#468b54';
        c.fillRect(0, 494, 900, 5);
        c.save();
        c.translate(175, s.y);
        c.rotate(Math.max(-0.4, Math.min(0.7, s.velocity / 800)));
        c.fillStyle = '#ffd45b';
        c.beginPath();
        c.ellipse(0, 0, 21, 17, 0, 0, Math.PI * 2);
        c.fill();
        c.fillStyle = '#e9a732';
        c.beginPath();
        c.ellipse(-8, 6, 12, 6, -0.2, 0, Math.PI * 2);
        c.fill();
        c.fillStyle = '#f28349';
        c.beginPath();
        c.moveTo(17, 0);
        ...
        c.textAlign = 'center';
        c.fillStyle = '#17485c';
        c.fillText(`${s.score} / ${s.stage.goal}`, 450, 46);
    }
    dispose() { this.disposed = true; this.sim.stop(); cancelAnimationFrame(this.frameId); }
}
