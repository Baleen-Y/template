...
const lightEnd = () => ({ message: 'stop', actions: [light('OFF')] });
export const STAGES = [
        ...
        for (let b = first; b; b = b.next?.block) {
            switch (b.type) {
                case 'flappy_light':
                    ...
                    break;
                case 'flappy_drive_pulse':
                    inspect(b, [], ['DIR', 'SPEED', 'MS']);
                    result.push({ kind: 'pulse', direction: pick(b, 'DIR', ['FORWARD', 'BACKWARD']), speed: number(b, 'SPEED', 0, 50), ms: number(b, 'MS', 20, 300) });
                    break;
                case 'flappy_park':
                    inspect(b, [], []);
                    result.push({ kind: 'park' });
                    break;
                case 'flappy_motor':
...
export function registerBlocks(B) {
    ...
    B.defineBlocksWithJsonArray([
        ...
        { type: 'flappy_drive_pulse', message0: 'roll %1 at %2 for %3 ms • then stop', args0: [
                ...
                { type: 'field_number', name: 'MS', value: 120, min: 20, max: 300, precision: 1 }
            ], ...statement, colour: 28,
...
export function toolbox(stage) {
    const cat = (name, colour, names) => ({ kind: 'category', name, colour, contents: names.map(type => ({ kind: 'block', type })) });
    const contents = [cat('When', '#547ab9', ['flappy_device_program', 'flappy_on_message']), cat('Lights', '#329e84', ['flappy_light'])];
    if (stage.id >= 2)
        contents.push(cat('Repeat', '#8a68c1', ['flappy_device_repeat', 'flappy_delay']));
    if (stage.id >= 3)
        contents.push(cat('Wheels', '#c78036', ['flappy_drive_pulse', 'flappy_park']));
